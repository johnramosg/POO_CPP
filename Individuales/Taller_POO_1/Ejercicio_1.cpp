...
*/
#include <iostream>  //Incluimos la libreria estandar para usar los dispopsitvos de entrada o salida
...
int main() // Definimos la funcion principal
{
  ...
  for (int i = 0; i <= 5; i++)     // Definimos un bucle for que en el cual inicializamos el iterador i, el cual se detiene cuando el iterador sea mayor que 5 aumenta de uno en uno
  {
    cout << i;                   // Mostramos el iterador en pantalla
    for (int j = 0; j <= 5; j++) // Definimos un bucle for que en el cual inicializamos el iterador j, el cual se detiene cuando el iterador sea mayor que 5 aumenta de uno en uno
    {
      ...
      cout << endl;   // Hacemos un salto de linea para que cuando termine cada fila de tablas la otra se muestre abajo
    }
